...
class PaymentTransaction(models.Model):
    ...
    def _send_capture_request(self):
        ...
        capture_response = self.acquirer_id._novalnet_make_request("transaction/capture",data=capture_payload)
        self._log_message_on_linked_documents(_portal_comments)
        self._handle_feedback_data('novalnet', {'nn_tid' : self.acquirer_reference,'portal_comments':_portal_comments})
    ...
    def _send_payment_request(self):
        ...
        self._handle_feedback_data('novalnet', processing_val)
    ...
    def _send_void_request(self):
        ...
        void_response = self.acquirer_id._novalnet_make_request("transaction/cancel",data=void_payload)
        self._log_message_on_linked_documents(_portal_comments)
        self._handle_feedback_data('novalnet', {'nn_tid' : self.acquirer_reference,'portal_comments':_portal_comments,'nn_status':'DEACTIVATED'})
    ...
    def _send_refund_request(self, amount_to_refund=None):
        ...
        refund_response = self.acquirer_id._novalnet_make_request("transaction/refund",data=refund_payload)
        ...
        if 'transaction' not in refund_response or 'tid' not in refund_response.get('transaction'):
            ...
            refund_tx._log_message_on_linked_documents(_portal_comments)
        refund_tx._handle_feedback_data('novalnet', notification_data)
        return refund_tx
    ...
    def _get_specific_processing_values(self, processing_values):
        ...
        self._log_message_on_linked_documents(_(
            ...
            provider_name=self.acquirer_id.name,payment_type=self.novalnet_payment_method.name, ref=self.reference
        ))
        payload = self._novalnet_prepare_payment_request(processing_values)

        endpoint = self._novalnet_prepare_end_point()
        payment_response = self.acquirer_id._novalnet_make_request(endpoint,data=payload)
        ...
        if self.operation in ['online_direct','online_token','offline']:
            ...
            self.acquirer_reference = payment_response.get('transaction')['tid']
    ...
    def _novalnet_prepare_tokenized_params(self):
        ...
        if self.token_id:
            payload = {'paydata':{'token':self.token_id.acquirer_ref}}
    ...
    def _initiate_transaction_callback(self,notification_data):
        ...
        if nn_ip != request_ip and not self.acquirer_id.novalnet_webhook_testing:
    ...
    def _create_transaction_payload(self,processing_values):
        ...
        if self.novalnet_payment_method.payment_term_id:
            ...
            if ipt and ipt.sudo().id != self.novalnet_payment_method.payment_term_id.id:
                ...
                terms = self.novalnet_payment_method.payment_term_id.compute(self.amount,date_ref=due_date,currency=self.currency_id)

                for balance_val in terms:
                    due_date = balance_val[0]
    ...
    def _get_tx_from_feedback_data(self, provider_code, notification_data):
        ...
        tx = self.search([('reference', '=', reference), ('provider', '=', 'novalnet')])
        if not tx:
    ...
    def _update_note_content(self):
        ...
        if self.invoice_ids:
            for inv in self.invoice_ids:
                inv.write({'narration': inv.narration + ' \n ' + self.env['ir.ui.view'].sudo().with_context(lang=lang)._render_template(
                            "payment_novalnet.novalnet_payment_information",
                            {'payment_tx_id': self.with_context(lang=lang), 'order': inv}
                        ),'invoice_payment_term_id': self.novalnet_payment_method.payment_term_id})
    ...
    def _process_feedback_data(self, notification_data):
        ...
        retrieve_transaction = self.acquirer_id._novalnet_make_request("transaction/details",data={'transaction':{'tid':notification_data.get('nn_tid')},'custom': { 'lang': self.env.context.get('lang')}})
        ...
        self.acquirer_reference = self.acquirer_reference or notification_data.get('nn_tid')
