...
class Paymentacquirer(models.Model):
    ...
    provider = fields.Selection(
        selection_add=[('novalnet', 'Novalnet AG')], ondelete={'novalnet': 'set default'}
    )
    ...
    novalnet_payment_methods_ids = fields.One2many('novalnet.payment.method', 'acquirer_id')
    ...
    def _get_default_payment_method_id(self):
        ...
        return self.env.ref('payment_novalnet.payment_method_novalnet').id

    @api.model
    def _is_tokenization_required(self, provider=None, **kwargs):
        if provider != 'novalnet':
            return super()._is_tokenization_required()
        return True
    ...
    def _get_partner_info_checkout(self,partner_id):
        return self.env['res.partner'].sudo().search([('id', '=', partner_id)])
    ...
    def _novalnet_get_available_payments(self, order, invoice, amount, currency, partner_id, invoice_id):

        if invoice_id:
            invoice = request.env['payment.acquirer'].sudo().filtered(lambda inv: inv.id ==invoice_id)
        ...
        if payment_guaranteed:
            ...
            if order and ( partner_id != order.partner_shipping_id.id or currency.name != 'EUR' or converted_amount <= 999):
                methods = methods - payment_guaranteed
            elif invoice and invoice_id!= invoice.partner_shipping_id.id or currency.name != 'EUR' or converted_amount <= 999:
                methods = methods - payment_guaranteed
            else:
    ...
    def get_novalnet_access_details(self):
        if not self.novalnet_public_key or not self.novalnet_key_password or self.provider != 'novalnet':
