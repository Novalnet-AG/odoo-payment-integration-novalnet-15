...
class NovalnetCallback(models.Model):
    ...
    transaction_id = fields.Many2one( string="Payment transaction", comodel_name='payment.transaction', readonly=True, domain='[("acquirer_id", "=", "acquirer_id")]', ondelete='restrict')
    ...
    def _process_refund(self,data):
        ...
        if self.transaction_id.refunds_count >0:
            ...
            refund_tx_from_nn_tid = refund_tx_from_source.filtered(lambda tx: tx.acquirer_reference == self.tid)
            if _shop_invoked or (refund_tx_from_nn_tid and refund_tx_from_nn_tid[0].acquirer_reference != self.transaction_id.acquirer_reference):
        ...
        if refund_tx:
            refund_tx.acquirer_reference = self.tid
            refund_tx._set_done()
    ...
    def _send_callback_email(self,comment):
        if not self.transaction_id.acquirer_id.novalnet_webhook_email:
            return
        mail_template = request.env.ref('payment_novalnet.novalnet_callback_notification').sudo()
        _subject = 'Novalnet odoo callback script'
        _email_to = self.transaction_id.acquirer_id.novalnet_webhook_email
