...
class PaymentNovalnetController(http.Controller):
    ...
    def novalnet_simulate_payment(self, **data):
        ...
        """
        _logger.info(data)
        request.env['payment.transaction'].sudo()._handle_feedback_data('novalnet', data)
    ...
    def novalnet_webhook(self, **data):
        ...
        payment_provider = request.env['payment.acquirer'].sudo().search([('provider','=','novalnet')])
        ...
        transaction_info._handle_feedback_data('novalnet', {'nn_tid':tid, 'event_type':event_type, 'check_sum': checksum, 'nn_status': data.get('result')['status'], 'nn_status_text': data.get('result')['status_text'], 'reference':transaction_info.reference})
    ...
    def novalnet_return_payment(self, **data):
        ...
        transaction_info._handle_feedback_data('novalnet', {'nn_tid':data['tid'],'nn_status': _nn_status, 'nn_status_text': _nn_status_text, 'reference':transaction_info.reference})
